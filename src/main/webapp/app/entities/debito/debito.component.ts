...
export class DebitoComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.debitoService
      ...
      .subscribe(
        (res: HttpResponse<IDebito[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInDebitos(): void {
    this.eventSubscriber = this.eventManager.subscribe('debitoListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IDebito[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/debito'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
